...
static unsigned char bmp085Read(unsigned char address)
{
...
// Calculate temperature in deg C x 10
static int bmp085GetTemperature(unsigned int ut){
  long x1, x2;
...
void pressureTempSensorSetup(){
  Wire.begin();
  bmp085Calibration();
}

void pressureTempSensorGetValues(int &temperature10, float &pressure, float &altitude) {
  temperature10 = bmp085GetTemperature(bmp085ReadUT()); //MUST be called first to compute b5
